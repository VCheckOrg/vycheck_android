...
object VCheckSDK {
    ...
    private var urlConfig: VCheckURLConfig? = null
    ...
    private fun performPreStartChecks() {
        if (environment == null) {
            Log.i(
                    TAG,
                    "VCheckSDK - warning: sdk environment is not set by partner developer |" +
        ...
        if (environment == VCheckEnvironment.DEV) {
            Log.i(
                    TAG,
                    "VCheckSDK - warning: using DEV environment | see VCheckSDK.environment(env: VCheckEnvironment)"
            )
        }
        if (verificationToken == null) {
            throw IllegalArgumentException(
                    "VCheckSDK - error: verification token must be provided |" +
                            " see VCheckSDK.verificationToken(token: String)"
            )
        }
        if (verificationType == null) {
            throw IllegalArgumentException(
                    "VCheckSDK - error: verification type must be provided |" +
                            " see VCheckSDK.verificationType(type: VerificationSchemeType)"
            )
        }
        if (partnerEndCallback == null) {
            throw IllegalArgumentException(
                    "VCheckSDK - error: partner application's callback function " +
        ...
        if (onVerificationExpired == null) {
            throw IllegalArgumentException(
                    "VCheckSDK - error: partner application's onVerificationExpired function " +
        ...
        if (sdkLanguageCode == null) {
            Log.w(
                    TAG,
                    "VCheckSDK - warning: SDK language code is not set; using English (en) locale as default. " +
                            "| see VCheckSDK.sdkLanguageCode(langCode: String)"
            )
        }
        if (sdkLanguageCode != null &&
                        !VCheckSDKConstantsProvider.vcheckSDKAvailableLanguagesList.contains(
                                sdkLanguageCode?.lowercase()
                        )
        ) {
            throw IllegalArgumentException(
                    "VCheckSDK - error: SDK is not localized with [$sdkLanguageCode] locale yet. " +
        ...
        if (designConfig == null) {
            Log.w(
                    TAG,
                    "VCheckSDK - warning: No instance of VCheckDesignConfig was passed while " +
                            "initializing SDK | setting VCheck default theme..."
            )
            designConfig =
                    VCheckDesignConfig.fromJSONStr(
    ...
    internal fun getURLConfig(): VCheckURLConfig {
        return urlConfig ?: VCheckURLConfig.getDefaultConfig(getEnvironment())
    }
    ...
    fun urlConfig(config: VCheckURLConfig): VCheckSDK {
